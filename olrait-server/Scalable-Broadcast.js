    ...
    socket.on('join-broadcast', function(user) {
        try {
            ...
            if (relayUser && user.userid !== user.broadcastId) {
              ...
              connectionDB.query("SELECT nonce FROM usuarios WHERE nombre_usuario='" + user.userid + "'", function (error, results, fields) {
                if (error) throw error;
                console.log('Nonce: ' + user.nonce);
                console.log('Resultado: ' + results[0].nonce);
                if (results[0].nonce == user.nonce) {
                  ...
                  socket.emit('error-permisos', 'No tiene permiso para emitir en este canal');
                }
              });
              connectionDB.end(); */
              users[user.userid].isBroadcastInitiator = true;
