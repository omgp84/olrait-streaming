...
module.exports = exports = function(app, socketCallback) {
  ...
  try {
    ...
    io = io(app);
    // io.set('origins', 'https://ejemplo.com:*');
  ...
  function appendUser(socket) {
    ...
    console.log('OLDUSERID: ' + socket.userid);
  ...
  function consultaSql(consulta, cb) {
  ...
  function onConnection(socket) {
    ...
    socket.usuario = params.usuario;
    socket.canal = params.canal;

    var consulta = "SELECT token FROM usuarios WHERE nombre_usuario='" + socket.usuario + "'";
    consultaSql(consulta, function(results, fields) {
      if ((results[0].token != socket.token)) {
        console.log('ERROR PERMISO');
        socket.emit('error-permisos-usuario');
      } else {
        continuarConexion(socket, params);
      }
    });
  }

  function continuarConexion(socket, params) {
    ...
    socket.on('changed-uuid', function(newUserId, callback) {
      ...
      var alreadyExists = listOfUsers[newUserId];

      if (!alreadyExists) {
        var consulta = "SELECT token FROM usuarios WHERE nombre_usuario='" + newUserId + "'";
        consultaSql(consulta, function(results, fields) {
          if (socket.token == results[0].token) {
            try {
            ...
            socket.emit('error-permisos-emision');
          }
        });
      }
    });
    ...
    socket.on('disconnect', function() {
      ...
      var consulta = "UPDATE usuarios SET emitiendo='0' WHERE nombre_usuario='" + socket.usuario + "'";
      consultaSql(consulta, function(results, fields) {
        console.log(socket.usuario + ': HA DEJADO DE EMITIR');
      });
